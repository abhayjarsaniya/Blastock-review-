        ...
        @if (is_null($hidden_menu_items) || !in_array('Categories', $hidden_menu_items))
          <li>
            ...
            <ul class="menu-cat hide">
              ...
              @foreach ($all_categories as $catGroup)
                <div class="mega-dropdown common-dropdown" data-simplebar data-category-id="{{ $catGroup->id }}" 
                @if ($catGroup->backgroundImage)
                  style="background-image: url('{{ get_storage_file_url(optional($catGroup->backgroundImage)->path, 'full') }}')"
                @endif
                >
